...
const base = process.env.WEBHOOK_WORKER_URL;
const secret = process.env.CRON_SECRET;
if (!base || !secret) throw new Error('WEBHOOK_WORKER_URL and CRON_SECRET are required');
const endpoint = new URL('/api/cron/webhooks', base);
...
do {
  try {
    ...
    console.info('Webhook sweep completed', { processed: result.processed });
  } catch {
    if (!shutdown.signal.aborted) {
      console.error('Webhook sweep failed; check service health and cron configuration');
