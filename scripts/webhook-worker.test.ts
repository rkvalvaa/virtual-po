...
async function invokeWorker(status: number) {
  ...
  try {
    const child = spawn(process.execPath, [resolve('scripts/webhook-worker.mjs'), '--once'], {
      env: { ...process.env, WEBHOOK_WORKER_URL: `http://127.0.0.1:${address.port}`, CRON_SECRET: 'worker-test-secret' },
...
it('runs the supported one-shot worker against its authenticated endpoint and exits', async () => {
  ...
  expect(result.requests).toEqual([{ path: '/api/cron/webhooks', authorization: 'Bearer worker-test-secret' }]);
...
it.each([503, 302])('reports HTTP %s as a failed worker invocation without leaking credentials or following redirects', async status => {
  ...
  expect(result.output).toContain('Webhook sweep failed');
  expect(result.output).not.toContain('worker-test-secret');
});
