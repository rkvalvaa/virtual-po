...
test("intake chat streams replies from the mocked Anthropic API", async ({
  ...
  const replies = page.getByRole("main").getByText(MOCK_REPLY)
  ...
  await input.fill("Unsent context that should survive navigation")
  await expect(send).toBeEnabled()
  await page.reload()
  await expect(replies).toHaveCount(2)
  await expect(input).toHaveValue("Unsent context that should survive navigation")
